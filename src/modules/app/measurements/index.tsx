...
export default function MeasurementFieldsPage() {
  ...
  const fields = useMemo(() => data?.data ?? [], [data?.data]);
  ...
  const stats = useMemo(() => {
    ...
    const decimal = fields.filter(
      (item) => item.inputType === "DECIMAL",
    ).length;
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="mx-auto flex w-full max-w-screen-2xl flex-col gap-4 p-4 md:p-6">
            <DashboardPageHeader
              ...
              description="Manage category-wise fields like Chest, Waist, Length, Size, and Shoulder."
              actions={
                <>
                  <Button
                    type="button"
            ...
            <Card className="flex min-h-0 flex-1 flex-col gap-0 overflow-hidden">
              <CardHeader className="border-b">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <CardTitle>Measurement Field Directory</CardTitle>
    ...
    <div className="min-w-262.5">
      ...
      {fields.map((field) => (
        ...
        >
          ...
          <p className="font-bold text-slate-800">{field.unit || "No unit"}</p>

...
import type { InvoiceLineItem } from "../api/useGetInvoices";
...
export function OrderDetailsDialog({
  ...
  const filteredLines =
    order?.orderLines?.filter((line: InvoiceLineItem) =>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
            ...
            <div className="md:col-span-2 space-y-1">
              ...
              <p className="text-[11px] font-bold text-slate-700 leading-normal wrap-break-word">
                {order.invoiceDate || "No address provided"}
              </p>
            </div>
              ...
              <div>
                <div
                  className={cn(
                    ...
                    order.portalStatus === "Invoiced"
                      ? "bg-emerald-100 text-emerald-700"
                      : "bg-blue-100 text-blue-700",
                  )}
                >
                  {order.portalStatus || "Unknown"}
                </div>
              </div>
            </div>
          </div>
